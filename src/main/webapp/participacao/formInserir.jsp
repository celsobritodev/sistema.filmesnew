...
<body>
	...
	<div class="container">
		<div class="page-header">
			<h1>Inserir nova participacao para o filme: ${participacao.filme.titulo}</h1>
		</div>
		...
		<form method="post" name="myform" class="form-horizontal"
			...
			<input type="hidden" name="codFilme" value="${participacao.filme.codFilme}" />
			...
			<div class="form-group">
				...
				<div class="col-sm-5">
					<input type="text" name="personagem" id="personagem"
						value="${participacao.personagem}" class="form-control" />
				</div>
			</div>
			<div class="form-group">
				...
				<div class="col-sm-5">
					<select name="codArtista" class="form-control">
						...
						<c:forEach items="${artistas}" var="artista">
							<option value="${artista.codArtista}" <c:if test="${artista == artistaSelecionado}">selected="selected"</c:if>>
								${artista.nome}, <fmt:setLocale value="pt_BR" /><fmt:formatNumber type="currency" value="${artista.cache}" />    
							</option>
						</c:forEach>
					</select>
				</div>
